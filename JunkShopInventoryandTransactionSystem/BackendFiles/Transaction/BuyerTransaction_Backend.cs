...
namespace JunkShopInventoryandTransactionSystem.BackendFiles.Transaction.BuyerLogic
{
    public class BuyerTransaction_Backend
    {
        // adds an item to the cart for buyer transaction
        public static bool HandleBuyerTransaction(
            int passedItemId,
            ...
            )
        {
            ...
            InventoryItem? item = reader.GetItemForBuyer(itemId); // Fetch only selling price
            ...
            if (item != null)
            {
                ...
                targetDataGridView.Rows.Add(
                    ...
                    item.itemName,
                    item.itemQtyType,
                    item.itemSellingPrice,  // Buyer sees Selling Price
                    exchangeQty,
                    exchangeAmount
                );
            }
            ...
            {
                MessageBox.Show("Item not found for Buyer Transaction.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            return true; // Return true if buyer transaction was successful
        }

        //finalizes the transaction for buyer
